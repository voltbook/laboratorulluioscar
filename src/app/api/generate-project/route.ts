...
const localProject = (idea: string) => {
  ...
  const weather = lower.includes("meteo") || lower.includes("vreme") || lower.includes("temperatur");
  const robot = lower.includes("robot") || lower.includes("motor") || lower.includes("obstacol");
  const title = garden ? "Seră inteligentă ESP32" : weather ? "Stație meteo ESP32" : robot ? "Robot autonom ESP32" : titleFromIdea(idea);
  ...
  const parts = garden
    ...
    : weather
      ? [
          { name: "ESP32 DevKit", category: "MCU", quantity: 1, estimatedPriceRon: 45, supplier: "OptimusDigital", supplierUrl: "https://www.optimusdigital.ro", description: "Controler Wi-Fi pentru publicarea datelor." },
          { name: "BME280 temperatură/umiditate/presiune", category: "Sensor", quantity: 1, estimatedPriceRon: 39, supplier: "Robofun", supplierUrl: "https://www.robofun.ro", description: "Senzor meteo compact pe I2C." },
          { name: "Senzor lumină BH1750", category: "Sensor", quantity: 1, estimatedPriceRon: 18, supplier: "Mivarom", supplierUrl: "https://www.mivarom.ro", description: "Măsoară iluminarea ambientală." },
          { name: "Panou solar 6V + charger TP4056 protejat", category: "Power", quantity: 1, estimatedPriceRon: 58, supplier: "eMAG", supplierUrl: "https://www.emag.ro", description: "Alimentare autonomă joasă tensiune." },
          { name: "Cutie IP65 mică", category: "Enclosure", quantity: 1, estimatedPriceRon: 42, supplier: "Dedeman", supplierUrl: "https://www.dedeman.ro", description: "Carcasă pentru exterior, cu găuri protejate." },
        ]
    : [
  ...
  return {
    ...
    category: garden ? "Automatizare" : weather ? "IoT meteo" : robot ? "Robotică" : "DIY hardware",
    difficulty: garden ? "Mediu" : "Începător",
    imagePrompt: `professional product render of ${title}, Romanian DIY hardware kit, clean studio background, visible electronics, safe low voltage, dark cyber laboratory accent`,
    conceptImageUrl: `/api/placeholder?title=${encodeURIComponent(title)}&accent=%2300ff66`,
    technicalDescription: garden
      ...
      : weather
        ? "Stație meteo autonomă pe ESP32 cu senzori I2C, alimentare solară joasă tensiune și carcasă protejată pentru măsurători de balcon sau grădină."
    ...
    tags: garden ? ["ESP32", "irigații", "senzori", "automatizare"] : weather ? ["ESP32", "meteo", "solar", "IoT"] : ["ESP32", "robotică", "motoare", "senzori"],
    estimatedCostRon: cost,
    parts,
    requiredTools: ["Multimetru", "Letcon cu vârf fin", "Cabluri Dupont", "Laptop cu Arduino IDE", "Clește de dezizolat", "Șurubelnițe fine"],
    assumptions: ["Utilizatorul lucrează doar la joasă tensiune.", "Modulele sunt cumpărate ca plăci gata fabricate.", "Testarea se face incremental, cu alimentare limitată în curent."],
    supplierSuggestions: ["eMAG", "Dedeman", "Leroy Merlin", "Hornbach", "TME", "Robofun", "OptimusDigital", "Mivarom", "Conex Electronic"],
    safetyWarnings: [
    ...
    wiringDiagram: garden
      ...
      : weather
        ? "flowchart LR\n  ESP32[ESP32 DevKit]\n  BME[BME280]\n  LUX[BH1750]\n  SOLAR[Panou solar 6V]\n  CHG[Charger TP4056 protejat]\n  BAT[Acumulator protejat]\n  ESP32 -- I2C --> BME\n  ESP32 -- I2C --> LUX\n  SOLAR --> CHG\n  CHG --> BAT\n  BAT --> ESP32"
      : "flowchart LR\n  ESP32[ESP32 DevKit]\n  TOF[Senzor VL53L0X]\n  DRIVER[Driver TB6612FNG]\n  M1[Motor stânga]\n  M2[Motor dreapta]\n  BAT[Baterie protejată]\n  ESP32 -- I2C --> TOF\n  ESP32 -- PWM/DIR --> DRIVER\n  DRIVER --> M1\n  DRIVER --> M2\n  BAT --> DRIVER\n  BAT --> ESP32",
    mechModel: {
      layers: ["structural", "electronics", "enclosure", "mounts", "power"],
      labels: garden ? ["rezervor", "pompă", "ESP32", "senzori sol"] : weather ? ["cutie IP65", "senzori", "panou solar", "ESP32"] : ["șasiu", "roți", "ESP32", "senzor frontal"],
    },
    instructionGroups: [
      {
        title: "Fabricare",
    ...
    testingSteps: ["Pornește numai cu sarcinile deconectate.", "Verifică tensiunile pe pini înainte de a monta actuatorii.", "Rulează test de stabilitate 30 minute.", "Notează temperaturile modulelor și consumul estimat."],
    troubleshooting: ["Dacă ESP32 se resetează, verifică sursa și vârfurile de curent.", "Dacă senzorii nu apar pe I2C, verifică SDA/SCL și adresele.", "Dacă actuatorul nu pornește, verifică GND comun și pinul de comandă."],
